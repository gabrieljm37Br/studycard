...
const Study: React.FC = () => {
    ...
    const evaluateAnswer = () => {
        ...
        if (card.mode === CardMode.TrueFalse) {
            ...
            evaluation = userSaysTrue === card.is_true ? 'correct' : 'incorrect';
        } else if (card.mode === CardMode.MultipleChoice) {
            // Multiple Choice: check if selected option matches correct index
            evaluation = selectedOption === card.correct_answer_index ? 'correct' : 'incorrect';
                                ...
                                <div>
                                    <div className={`p-6 rounded-xl border-2 mb-6 ${result === 'correct'
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {(currentCard.explanation || (currentCard.mode === CardMode.PracticalExample && currentCard.solution)) && (
                                                <div className="bg-white/50 dark:bg-black/20 p-4 rounded-lg">
                                                    <p className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">
                                                        Explicação
                                                    </p>
                                                    <p className="text-gray-700 dark:text-gray-300 leading-relaxed" dangerouslySetInnerHTML={renderHTML(currentCard.explanation || "Veja a solução acima.")} />
                                                </div>
                                            )}
