...
/**
 * Converte delimitadores LaTeX ($...$ ou $$...$$) em spans compatÃ­veis com KaTeX,
 * usados pelo editor e pelo viewer. Evita quebrar textos com nÃºmero Ã­mpar de
 * cifrÃµes para nÃ£o corromper strings que tenham $ literal.
 */
function embedLatex(text: string): string {
    ...
    if (dollarCount >= 2 && dollarCount % 2 === 0) {
        output = output.replace(/\$(.+?)\$/gs, (_match, expr) =>
            `<span data-latex="${escapeLatexAttr(expr.trim())}"></span>`
        );
    }
...
const keywordPatterns = {
    ...
    questaoLatex: '(?:Questao\s+Latex:|Questao\s+LaTeX:|Questão\s+Latex:|Questão\s+LaTeX:)',
    ...
    opcaoLatex: '(?:Opcao\s+Latex:|Opcao\s+LaTeX:|Opção\s+Latex:|Opção\s+LaTeX:)'
};
...
function parseQuestaoCard(blockText: string): ParsedAnkiCard {
    const questionPattern = new RegExp(`(?:${keywordPatterns.questao}|${keywordPatterns.questaoLatex})\\s*([\\s\\S]*?)(?=(?:${keywordPatterns.resposta}|${keywordPatterns.respostaLatex})|$)`, 'i');
    ...
    // OpÃ§Ãµes explÃ­citas usando "Opcao LaTeX:"
    const opcaoLatexRegex = new RegExp(`(?:^|\\n)\\s*(?:[-â€¢]\\s*)?${keywordPatterns.opcaoLatex}\\s*([\\s\\S]*?)(?=(?:\\n\\s*(?:[-â€¢]\\s*)?${keywordPatterns.opcaoLatex}|${keywordPatterns.resposta}|${keywordPatterns.respostaLatex}|$))`, 'gi');
    let opcaoMatch;
    ...
    // Ajuste para 4 opÃ§Ãµes: truncar ou preencher
...
function detectCardType(blockText: string): { type: string; keyword: string } | null {
    ...
    const normalizedStart = trimmed.replace(/^[\uFEFF"'Ã¢â‚¬Å“Ã¢â‚¬ÂÃ¢â‚¬ËœÃ¢â‚¬â„¢\s]+/, '');
...
function splitIntoBlocks(content: string): string[] {
    ...
    const regex = new RegExp(`(?=[\\s"'Ã¢â‚¬Å“Ã¢â‚¬ÂÃ¢â‚¬ËœÃ¢â‚¬â„¢]*(?:${pattern}))`, 'gi');
...
export function validateAnkiTxtContent(content: string): { valid: boolean; error?: string } {
    if (!content || content.trim().length === 0) {
        return { valid: false, error: 'O arquivo TXT estÃƒÂ¡ vazio' };
    }
    ...
    if (blocks.length === 0) {
        return {
            valid: false,
            error: 'Nenhum flashcard vÃƒÂ¡lido encontrado. Certifique-se de que o arquivo contÃƒÂ©m cards com palavras-chave reconhecidas (Pergunta:, Certo ou Errado:, etc.)'
        };
    }
