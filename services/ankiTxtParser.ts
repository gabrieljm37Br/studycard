...
const keywordPatterns = {
    ...
    explicacao: '(?:Explicacao:|Explica��o:)',
    questao: '(?:Questao:|Quest�o:)',
    questaoLatex: '(?:Questao\s+Latex:|Questao\s+LaTeX:|Quest�o\s+Latex:|Quest�o\s+LaTeX:)',
    dicionario: '(?:Dicionario:|Dicion�rio:)',
    significado: '(?:Significado:)',
    certoErrado: '(?:Certo ou Errado:)',
    situacao: '(?:Situacao[- ]?Problema:|Situacao[- ]?Problema:|Situa��o[- ]?Problema:)',
    problema: '(?:Problema:)',
    hipotese: '(?:Hipotese:|Hipotese:|Hip�tese:)',
    tags: '(?:Tags:|Etiquetas:)',
    lacuna: '(?:Lacuna:|Lacunas:|Lacuna:)',
    opcaoLatex: '(?:Opcao\s+Latex:|Opcao\s+LaTeX:|Op��o\s+Latex:|Op��o\s+LaTeX:)'
};
...
function detectCardType(blockText: string): { type: string; keyword: string } | null {
    ...
    const normalizedStart = trimmed.replace(/^[\uFEFF"'â€œâ€â€˜â€™\s]+/, '');
...
function splitIntoBlocks(content: string): string[] {
    ...
    const regex = new RegExp(`(?=[\\s"'â€œâ€â€˜â€™]*(?:${pattern}))`, 'gi');
...
export function validateAnkiTxtContent(content: string): { valid: boolean; error?: string } {
    if (!content || content.trim().length === 0) {
        return { valid: false, error: 'O arquivo TXT estÃ¡ vazio' };
    }
    ...
    if (blocks.length === 0) {
        return {
            valid: false,
            error: 'Nenhum flashcard vÃ¡lido encontrado. Certifique-se de que o arquivo contÃ©m cards com palavras-chave reconhecidas (Pergunta:, Certo ou Errado:, etc.)'
        };
    }
